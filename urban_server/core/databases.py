...
INIT_BURO_JSON: dict = Path("init_data/buro.json")
...
async def init_chromadb():
    ...
    if (await collection.count()) == 0:
        bureaus = json.loads(INIT_BURO_JSON.read_text())

        valid = lambda k, v: (0 if k == "year" else "-") if v is None else v
        for bureau in bureaus:
            tags = ", ".join(["_".join(d.split(" ")) for d in bureau["tags"]])
            tags_vector = np.mean(
                await embeddings.aembed_documents(bureau["tags"]), axis=0
            ).tolist()
            metadata = bureau
            metadata = {k: valid(k, v) for k, v in metadata.items()}
            metadata["tags"] = tags
            await collection.add(
                embeddings=[tags_vector], metadatas=[metadata], ids=[bureau["name"]]
            )
